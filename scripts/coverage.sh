...
/usr/local/opt/llvm/bin/llvm-cov show $CURDIR/../build/simplicial_arrangement_tests -instr-profile=$CURDIR/../build/simplicial_arrangement.profdata --ignore-filename-regex="_deps" --ignore-filename-regex="tests/"  $CURDIR/../src/*.cpp -use-color --format html > $CURDIR/../build/coverage.html
