...
void FastArrangement::run_with_timer(vector<std::string>& timing_labels, vector<double>& timings)
{
    auto t_begin = std::chrono::high_resolution_clock::now();
...
//    std::cout << "Arrangement: resolving self-intersection: " << resolve_time.count() << std::endl;
//    std::cout << "Arrangement: extracting arrangement: " << extract_time.count() << std::endl;
    timing_labels.emplace_back("arrangement(resolve intersection)");
    timings.push_back(resolve_time.count());
    timing_labels.emplace_back("arrangement(extract arrangement)");
    timings.push_back(extract_time.count());
